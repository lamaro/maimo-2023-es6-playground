...
const longestSong = songs.find(
  ({ title }) => title.length === Math.max(...songs.map(({title}) => title.length))
);

console.log('looongest soooooong', longestSong)
...
const randomSong = (max,min) => songs[Math.floor(Math.random()*(max - min) + min)]

console.log(randomSong(songs.length, 0))
console.log(randomSong(songs.length, 0))
console.log(randomSong(songs.length, 0))
console.log(randomSong(songs.length, 0))